...
def test_create_rating(client, test_enthusiast_headers):
    ...
    response = client.post("/ratings/", json=rating_data, headers=test_enthusiast_headers)
...
def test_create_rating_invalid_score(client, test_enthusiast_headers):
...
def test_create_rating_missing_fields(client, test_enthusiast_headers):
...
def test_get_user_ratings(client, test_enthusiast_headers):
    """Test getting all ratings by current user"""
    response = client.get("/ratings/my-ratings", headers=test_enthusiast_headers)
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, list)

def test_update_rating(client, test_enthusiast_headers):
    ...
    create_response = client.post("/ratings/", json=rating_data, headers=test_enthusiast_headers)
    ...
    response = client.put(f"/ratings/{rating_id}", json=update_data, headers=test_enthusiast_headers)
    assert response.status_code == 200
    result = response.json()
    assert result["rating"] == update_data["rating"]

def test_delete_rating(client, test_enthusiast_headers):
    ...
    # Delete the rating
    response = client.delete(f"/ratings/{rating_id}", headers=test_enthusiast_headers)
    assert response.status_code == 204

def test_duplicate_rating_prevention(client, test_enthusiast_headers):
    ...
    first_response = client.post("/ratings/", json=rating_data, headers=test_enthusiast_headers)
    if first_response.status_code != 201:
        pytest.skip("Could not create first rating")
    
    # Second rating for same painting should fail or update existing
    second_response = client.post("/ratings/", json=rating_data, headers=test_enthusiast_headers)
